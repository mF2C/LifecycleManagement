...
def deploy(service, agent):
    LOG.debug("LIFECYCLE: (internal) Operations: deploy: " + str(service) + ", agent: " + str(agent))
    return apps_adapter.deploy_service_agent(service, agent)


# Service Operation: start
def start(service, agent):
    LOG.info("LIFECYCLE: (internal) Operations: start service: " + str(service) + ", agent: " + str(agent))
    try:
        ...
        LOG.error('LIFECYCLE: (internal) Operations: start: Exception')
        return common.gen_response(500, 'Exception', 'agent', str(agent))


# Service Operation: stop
def stop(service, agent):
    LOG.info("LIFECYCLE: (internal) Operations: stop: " + str(service) + ", agent: " + str(agent))
    try:
...
def terminate(service, agent):
    LOG.info("LIFECYCLE: (internal) Operations: terminate: " + str(service) + ", agent: " + str(agent))
    try:
