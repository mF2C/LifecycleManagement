...
def deploy_docker_image(service, agent):
    LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_image] " + str(service) + ", " + str(agent))
    try:
        ...
        if container1 is not None:
            ...
            LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_image] container: " + str(container1))
            ...
            LOG.error("[lifecycle.modules.apps.docker.adapter] [deploy_docker_image] Could not connect to DOCKER API")
        ...
        LOG.exception('[lifecycle.modules.apps.docker.adapter] [deploy_docker_image] Exception')
...
def deploy_docker_compss(service, agent):
    ...
    try:
        ...
        if container1 is not None:
            ...
            LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compss] container: " + str(container1))
            ...
            LOG.error("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compss] Could not connect to DOCKER API")
            agent['status'] = STATUS_ERROR
            return common.gen_response(500, 'Error when connecting to DOCKER API', 'agent', str(agent), 'service', str(service))
    except:
        LOG.exception('[lifecycle.modules.apps.docker.adapter] [deploy_docker_compss] Exception')
...
def deploy_docker_compose(service, agent):
    LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] " + str(service) + ", " + str(agent))
    try:
        # 1. Download docker-compose.yml file
        location = service['exec']
        LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] Getting docker-compose.yml from " + location + " ...")
        # remove previous files
        try:
            os.remove(config.dic['WORKING_DIR_VOLUME'] + "/docker-compose.yml")
        except:
            LOG.warning("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] Error when removing file: " + config.dic['WORKING_DIR_VOLUME'] + "/docker-compose.yml")
        # download docker-compose.yml
        try:
            res, _ = urequest.urlretrieve(location, config.dic['WORKING_DIR_VOLUME'] + "/docker-compose.yml")
            LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] > download result: " + str(res))
        except:
            LOG.exception("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] Error when downloading file to: " + config.dic['WORKING_DIR_VOLUME'] + "/docker-compose.yml")
        ...
        if container1 is not None:
            LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] container1: " + str(container1))
            ...
            if container2 is not None:
                ...
                LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] container2: " + str(container2))
                LOG.debug("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] container '1' & '2' created")
                ...
                LOG.error("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] container '2' not created")
                agent['agent_param'] = "-"
            ...
            LOG.error("[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] Could not connect to DOCKER API")
            agent['status'] = STATUS_ERROR
            return common.gen_response(500, 'Error when connecting to DOCKER API', 'agent', str(agent), 'service',
                                       str(service))
    except:
        LOG.exception('[lifecycle.modules.apps.docker.adapter] [deploy_docker_compose] Exception')
...
def operation_service_agent(agent, operation):
    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] [" + operation + "]: " + str(agent))
    try:
        ...
        if docker_client.get_client_agent_docker() is not None:
            if operation == OPERATION_START:
                ...
                LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] docker-compose? [l_elem=" + str(l_elem) + "]")

                # docker-compose
                if l_elem is not None and l_elem['type'] == SERVICE_DOCKER_COMPOSE:
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] 'Docker-compose down' container [" + l_elem['container_2'] + "] launched ...")
                    docker_client.start_container(l_elem['container_2'])
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Executing 'docker-compose down' (waiting 60 seconds) ...")
                    time.sleep(60)
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Stopping 'Docker-compose up' container [" + agent['container_id'] + "] ...")
                    docker_client.stop_container(agent['container_id'])
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Stopping 'Docker-compose down' container [" + l_elem['container_2'] + "] ...")
                    docker_client.stop_container(l_elem['container_2'])
                    agent['status'] = STATUS_STOPPED
                # 'normal' container
                else:
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Stop container: " + agent['container_id'])
                ...
                # docker-compose
                if l_elem is not None and l_elem['type'] == SERVICE_DOCKER_COMPOSE:
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Remove container 1 [" + agent['container_id'] + "] ...")
                    docker_client.remove_container(agent)
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Remove container 2 [" + l_elem['container_2'] + "] ...")
                    docker_client.remove_container_by_id(l_elem['container_2'])
                # 'normal' container
                else:
                    LOG.debug("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Remove container: " + agent['container_id'])
                    docker_client.remove_container(agent)
                agent['status'] = STATUS_TERMINATED

        # if error when connecting to agent...
        else:
            LOG.error("[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Could not connect to DOCKER API")
            agent['status'] = STATUS_UNKNOWN
        # return status
        return agent['status']
    except:
        agent['status'] = STATUS_ERROR
        LOG.exception('[lifecycle.modules.apps.docker.adapter] [operation_service_agent] Exception')
        return STATUS_ERROR
