...
def deploy_docker_image(service, agent):
    ...
    try:
        ...
        if container1 is not None:
            ...
            LOG.error("LIFECYCLE: Docker adapter: deploy_docker_image: Could not connect to DOCKER API")
...
def deploy_docker_compss(service, agent):
    ...
    try:
        ...
        try:
            ...
            LOG.error("LIFECYCLE: Docker adapter: deploy_docker_compss: field 'master_compss' not found in agent")
        ...
        if container1 is not None:
            ...
            LOG.error("LIFECYCLE: Docker adapter: deploy_docker_compss: Could not connect to DOCKER API")
            agent['status'] = STATUS_ERROR
            return common.gen_response(500, 'Error when connecting to DOCKER API', 'agent', str(agent), 'service', str(service))
    except:
...
def deploy_docker_compose(service, agent):
    ...
    try:
        ...
        try:
            ...
            LOG.debug("LIFECYCLE: Docker adapter: deploy_docker_compose:  > download result: " + str(res))
        except:
...
def deploy_service_agent(service, agent):
    LOG.debug("LIFECYCLE: Docker adapter: (1) deploy_service_agent: " + str(service) + ", " + str(agent))
    try:
        # docker-compose
        ...
        # not defined
            ...
            LOG.warning("LIFECYCLE: Docker adapter: deploy_service_agent: [" + service['exec_type'] + "] not defined")
