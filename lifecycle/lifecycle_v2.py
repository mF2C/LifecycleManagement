"""
Lifecycle v2: deployment in 'parallel' mode
...
def check_service_content(service):
    if 'category' not in service or 'exec_type' not in service or 'exec' not in service:
        LOG.debug("Lifecycle-Management: Lifecycle2: check_service_content: fields category/exec/exec_type not found")
        return False
    return True
...
def thr_submit_local(service, agent):
    try:
        LOG.debug("Lifecycle-Management: Lifecycle2: thr_submit_local: allocate service locally")
        ...
        if agent['status'] == "waiting":
            LOG.debug("Lifecycle-Management: Lifecycle2: thr_submit_local: execute service locally")
            # executes service
            execution_adapter.execute_service_agent(service, agent)
        else:
            LOG.error("Lifecycle-Management: Lifecycle2: thr_submit_local: allocate service locally: NOT DEPLOYED")
            agent['status'] = "not-deployed"

    except:
        LOG.error("Lifecycle-Management: Lifecycle2: thr_submit_local: thr: Exception")



def thr_submit_remote(service, agent):
    try:
        LOG.debug("Lifecycle-Management: Lifecycle2: thr_submit_remote: allocate service in remote agent [" + agent['url'] + "]")
        ...
        if resp_deploy is not None:
            ...
            LOG.debug("Lifecycle-Management: Lifecycle2: thr_submit_remote: allocate service in remote agent: "
                      "[agent=" + str(agent) + "]")
            ...
            if resp_start is not None:
                ...
                LOG.debug("Lifecycle-Management: Lifecycle2: thr_submit_remote: execute service in remote agent: "
                          "[agent=" + str(agent) + "]")
        else:
            LOG.error(
                "Lifecycle-Management: Lifecycle2: thr_submit_remote: allocate service in remote agent: NOT DEPLOYED")
            agent['status'] = "not-deployed"

    except:
        LOG.error("Lifecycle-Management: Lifecycle2: thr_submit_remote: thr: Exception")



# thr_submit_service_in_agents
def thr_submit_service_in_agents(service, service_instance, user_id, agreement_id):
    try:
        LOG.debug("Lifecycle-Management: Lifecycle2: thr_submit_service_in_agents: " + str(service) + ", user_id: " + user_id)
        ...
        LOG.debug("Lifecycle-Management: Lifecycle2: submit_service_in_agents: service_instance: " + str(service_instance))
        ...
        for agent in service_instance["agents"]:
            ...
            # NOT FOUND / NOT CONNECTED
                ...
                agent['status'] = "error"
                LOG.error("Lifecycle-Management: Lifecycle2: submit_service_in_agents: agent [" + agent['url'] + "] cannot be reached")
        ...
        if sla_adapter.initializes_sla(service_instance, agreement_id):
            LOG.debug("Lifecycle-Management: Lifecycle2: submit_service_in_agents: sla agreement started")
        else:
            LOG.error("Lifecycle-Management: Lifecycle2: submit_service_in_agents: sla agreement NOT started")

        # 6. save / update service_instance
        LOG.debug("Lifecycle-Management: Lifecycle2: submit_service_in_agents: UPDATING service_instance: " + str(service_instance))
        service_instance['status'] = STATUS_STARTED
        data.update_service_instance(service_instance['id'], service_instance)

        LOG.debug("Lifecycle-Management: Lifecycle2: submit_service_in_agents: service instance deployed: service_instance: " + str(service_instance))
    except:
        LOG.error("Lifecycle-Management: Lifecycle2: thr_submit_service_in_agents: thr: Exception")
...
def submit_service_in_agents(service, user_id, agreement_id, agents_list, check_service=False):
    LOG.debug("Lifecycle-Management: Lifecycle2: submit_service_in_agents: " + str(service) + ", user_id: " + user_id)
    try:
        ...
        LOG.debug("Lifecycle-Management: Lifecycle2: submit_service_in_agents: agents_list" + str(agents_list))

        service_instance = data.create_service_instance(service, agents_list, user_id, agreement_id)
        if not service_instance:
            LOG.error("Lifecycle-Management: Lifecycle2: submit_service_in_agents: error creating service_instance")
        ...
        LOG.error('Lifecycle-Management: Lifecycle2: submit_service_in_agents: Exception')
...
def submit(service, user_id, agreement_id):
    LOG.debug("Lifecycle-Management: Lifecycle2: submit_v2: " + str(service) + ", user_id: " + user_id)
    try:
        # 1. check parameters content
        ...
        if not available_agents_list:
            # error
            LOG.error("Lifecycle-Management: Lifecycle2: submit_v2: available_agents_list is None")
            ...
            LOG.error("Lifecycle-Management: Lifecycle2: submit_v2: available_agents_list is empty")
        ...
        LOG.error('Lifecycle-Management: Lifecycle2: submit_v2: Exception')
        return common.gen_response(500, 'Exception', 'service', str(service))
